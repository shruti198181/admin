...
export default function Report() {
  ...
  const handleEdit = (id) => {
    alert(`Edit functionality for todo ID: ${id}`);
  };

  const handlePreview = (id) => {
    alert(`Preview functionality for todo ID: ${id}`);
  };
        ...
        <Table className="border border-gray-300 border-collapse min-w-[800px] mx-auto scrollbar-none">
          <TableHeader>
            <TableRow className="bg-gray-100">
              ...
              <TableHead className="border p-2 text-left">Title</TableHead>
              <TableHead className="border p-2 text-center whitespace-nowrap">Completed</TableHead>
              <TableHead className="border p-2 text-center whitespace-nowrap">Action</TableHead> {/* New column */}
            </TableRow>
          </TableHeader>
              ...
              <TableRow key={todo.id} className="hover:bg-gray-50">
                ...
                <TableCell className="border p-2 text-center">
                  <button
                    ...
                    className={`w-40 py-1 rounded-full font-semibold text-white text-center transition-colors
                      ${todo.completed ? "bg-green-600 hover:bg-green-700" : "bg-red-600 hover:bg-red-700"}`}
                  >
                    {todo.completed ? "Completed" : "Not Completed"}
                  </button>
                ...
                <TableCell className="border p-2 text-center flex justify-center gap-2">
                  <button
                    ...
                    className="px-2 py-1 rounded bg-blue-500 hover:bg-blue-600 text-white text-sm"
                  >
                    Edit
                  </button>
                  <button
                    onClick={() => handleDelete(todo.id)}
                    className="px-2 py-1 rounded bg-red-500 hover:bg-red-600 text-white text-sm"
                  >
                    Delete
                  </button>
                  <button
                    onClick={() => handlePreview(todo.id)}
                    className="px-2 py-1 rounded bg-gray-500 hover:bg-gray-600 text-white text-sm"
                  >
                    Preview
                  </button>
                </TableCell>
